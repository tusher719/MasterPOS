...
class ExpenseCategoryImport implements ToCollection, WithHeadingRow, SkipsEmptyRows, WithValidation
{
    ...
    public function __construct(bool $isDryRun = true)
    {
        ...
        $this->existingNames = ExpenseCategory::pluck('name')
    ...
    private function processRow(array $row, int $rowNumber): array
    {
        ...
        if ($name === '') {
            ...
            $errors[] = "Expense category \"{$name}\" already exists — skipped.";
        }

        // Validate hex color if provided
        if ($color !== '' && !preg_match('/^#[0-9A-Fa-f]{6}$/', $color)) {
            $warnings[] = "Color \"{$color}\" is not a valid hex code — will be left blank.";
            $color = '';
        }
