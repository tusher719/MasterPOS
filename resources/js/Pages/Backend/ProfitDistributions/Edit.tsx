...
import { Calculator, Save, ArrowLeft, RefreshCw, Lock } from "lucide-react";
...
// -----------------------------------------------------------------------
// Convert a backend date value (which may arrive as a full ISO datetime
// string like "2026-07-05T00:00:00.000000Z" due to Laravel's `date` cast)
// into the "YYYY-MM-DD" format required by <input type="date">.
// Without this, the date input renders blank even though data exists.
// -----------------------------------------------------------------------
...
export default function Edit({ distribution, can }: Props) {
    // Guard — if somehow a locked record reaches this page, show a notice
    ...
    async function handleRecalculate() {
        ...
        try {
            ...
            if (data.items.length === 0) {
                ...
                toast.success(
                    `Recalculated — ${data.items.length} investor(s) found.`,
                );
            }
        } catch (err: any) {
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        if (form.items.length === 0) {
            toast.error("No investor items found. Please recalculate first.");
            return;
        }
